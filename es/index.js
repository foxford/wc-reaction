const withConfig = (base, initialConfig) => class extends base {
  constructor (props) {
    super(props);
...
class Reactions extends LitElement {
  ...
  _renderIcon (data) {
    return data.icon ? html$1`<img src="${data.icon}" />` : this.emoji.get(data.name)
  }
    ...
    return !shouldRender
      ...
      : html$1`
        <div class="icon" name=${k} on-click=${this._boundedClick}>
          ...
          ${shouldCount ? html$1`<span class="count">${count}</span>` : null}
        </div>
      `
  }
...
const ReactionList = withStyle(Reactions, css);

export { mixins, ReactionList };
